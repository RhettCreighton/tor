 ...
 */
typedef int (*hs_builtin_service_handler_t)(edge_connection_t *conn);
 ...
 *
 * @param conn The edge connection to process
 * @param virtual_port The virtual port from the client connection
 * @return 0 on success, -1 on error
 */
int hs_handle_builtin_service(edge_connection_t *conn, uint16_t virtual_port);

/**
 * Register default built-in service handlers.
 *
 * This registers the Hello World handler for ports 80 and 443 by default.
 */
